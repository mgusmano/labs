Ext.define('AppCamp.view.main.MainViewController', {
	...
	Route:function(xtype) {
		var node = this.lookup('menuview').getStore().findNode('xtype', xtype);
		if (node == null) {
		...
		//item = this.lookup('centerview').child('component[xtype=' + xtype + ']');
		item = this.lookup('centerview').getComponent(xtype);
		//same as application
		if (!item) {
				try {
